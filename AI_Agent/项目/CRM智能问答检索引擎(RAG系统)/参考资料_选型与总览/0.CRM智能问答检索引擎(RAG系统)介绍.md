...
CRM组的业务链路是：用户看到百度广告留资变成线索，销售跟进转化，客户下单后签合同进财务系统，运营同学再负责投放和售后。这条链路里，销售和运营同学每天要查大量文档——合同条款、产品政策、投放规则、售后流程，人工翻文档效率很低，于是CRM组启动了这个RAG项目，目标是让销售运营能直接问、直接拿到准确答案。

项目立项初期时间紧，需要一个能快速跑起来、能演示给老板看的版本，赶工做出了第一版，也就是graph1。架构上是LangGraph实现的一个简单Agentic RAG：LLM自己判断要不要调用检索工具，检索到文档后做一次相关性打分，相关就生成回答，不相关就改写问题重新决策，支持多轮对话历史。这版核心目的是验证"知识库+Agent"这个基础范式能不能跑通，跑通了就先上线给部分产品和开发同学开通权限试用。
...
详见 [`1.graph.md`](./1.graph.md) 开头"两条Pipeline的定位与演进关系"一节——graph1、graph2不是并列共存的两个方案，是一次技术方案演进的两个阶段，附带迭代动机的完整推演。
...
### 6.4 graph2完整流程图（Mermaid）

代码位置：`graph2/graph_2.py:104-147`（节点与边的注册代码）

```mermaid
flowchart TD
    START([START]) -->|route_question 路由判断| ROUTE{问题该走<br/>知识库还是网搜?}
    ROUTE -->|web_search| WEBSEARCH[web_search<br/>网络搜索]
    ROUTE -->|vectorstore| RETRIEVE[retrieve<br/>知识库检索]

    RETRIEVE --> GRADE_DOCS[grade_documents<br/>逐文档相关性打分+过滤]
    GRADE_DOCS --> DECIDE{decide_to_generate}
    ...
    GENERATE --> GRADE_GEN{grade_generation_v_<br/>documents_and_question}
    ...
    style GRADE_GEN fill:#ffe0e0
    style DECIDE fill:#e0f0ff
    style ROUTE fill:#e0f0ff
```

**图中三个判断点，对应Self-RAG论文的三次自我校验**（详见第7节）：
1. `ROUTE`（检索前）——该走知识库还是外部搜索
2. `DECIDE`（检索后）——文档是否真的相关，相关性不够时是重试改写还是降级web搜索
3. `GRADE_GEN`（生成后）——**两级校验顺序不能颠倒**：先判①有没有幻觉（是否基于`documents`），再判②是否切题（是否回答了`question`）。因为一段完全编造的内容完全可能"看起来很切题"，必须先确保生成有依据，再谈这个有依据的回答是否真正回答了问题。

**两条失败分支走向不同，是"按失败原因分类修复"而非笼统重试**：
- `"not supported"`（幻觉）→ 文档没换，回`generate`自身换一次生成尝试
- `"not useful"`（未切题）→ 可能是检索角度问题，走`transform_query`换个问题表达重新检索

**已知设计不一致（现存未修复问题）**：`DECIDE`那条循环有`transform_count`兜底收敛（改写2次仍无果就转web搜索降级），但`GRADE_GEN`的`"not supported"→generate`这条边**没有任何计数上限**——如果生成持续被判定为幻觉，会无限重生成下去。这是`transform_query`的收敛模式没有被系统性推广到`generate`重试的典型工程遗留问题，修复思路是加一个`generation_retry_count`字段，超阈值后降级返回"无法确定答案准确性"的兜底提示，而非无限占用资源空转（详见第9节难点3）。
